...
- [x] **Astra as `/codex` default:** done 2026-09-09 — `/codex` now defaults to `gpt-6-astra` at `medium` (Astra's catalogue default), matching `/autosequence`'s codex reviewer. Sol, Terra, and Luna stay reachable by name. Re-baseline the effort after a few runs if `medium` proves thin on hard problems.
