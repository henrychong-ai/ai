...
- [x] **Astra as `/codex` default:** done 2026-09-09 — `/codex` now defaults to `gpt-6-astra` at `medium` (Astra's catalogue default). Sol and Luna stay reachable by name. Re-baseline the effort after a few runs if `medium` proves thin on hard problems.
- [x] **GPT-6 Sol/Luna:** done 2026-09-24 — `sol` → `gpt-6-sol`, `luna` → `gpt-6-luna`; `terra` retired (no GPT-6 successor); Astra capacity fallback moved to `gpt-6-sol` at `xhigh` in the `codex-relay` agent.
